...
unsigned int nWorkTargetSpacing = 3 * 60; // 3 minutes per block
unsigned int nStakeTargetSpacing = 36; // 36 seconds
...
const string strMessageMagic = "TheMermaidCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
   ...
   if (pindexBest->nHeight <= 1)
      {
        int64_t nSubsidy = 75000 * COIN;
        return nSubsidy + nFees;
      }
      
    else if (pindexBest->nHeight <= 480)
      {
        int64_t nSubsidy = 0 * COIN;
        return nSubsidy + nFees;
      }
    ...
    else if (pindexBest->nHeight <= 3360)
      {
        int64_t nSubsidy = 1000 * COIN;
        return nSubsidy + nFees;
      }
	  
    else if (pindexBest->nHeight <= 16800)
      {
        int64_t nSubsidy = 0 * COIN;
        return nSubsidy + nFees;
      }
    ...
    else if (pindexBest->nHeight <= 17760)
      {
        int64_t nSubsidy = 10 * COIN;
        return nSubsidy + nFees;
      } 
    ...
    else if (pindexBest->nHeight <= 18720)
      {
        int64_t nSubsidy = 100 * COIN;
        return nSubsidy + nFees;
      }
    ...
    else if (pindexBest->nHeight <= 19680)
      {
        int64_t nSubsidy = 1000 * COIN;
        return nSubsidy + nFees;
      }
    ...
    else if (pindexBest->nHeight <= 26880)
      {
        int64_t nSubsidy = 0 * COIN;
        return nSubsidy + nFees;
      }	  
...
{  
	...
	if(pindexBest->nHeight > 3360)
    {
	    nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 365;
		return nSubsidy + nFees; 
    }
	
	if(pindexBest->nHeight > 6720)
    {
        nSubsidy = 1 * COIN; 
		return nSubsidy + nFees; 
    }
	...
	if(pindexBest->nHeight > 7680)
    {
        nSubsidy = 4 * COIN; 
		return nSubsidy + nFees; 
    }
	if(pindexBest->nHeight > 8160)
    {
        nSubsidy = 5 * COIN; 
		return nSubsidy + nFees; 
    }
	if(pindexBest->nHeight > 8640)
    {
        nSubsidy = 6 * COIN; 
		return nSubsidy + nFees; 
    }
	if(pindexBest->nHeight > 9120)
    {
        nSubsidy = 7 * COIN; 
		return nSubsidy + nFees; 
    }
	if(pindexBest->nHeight > 10080)
    {
	    nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 730;
		return nSubsidy + nFees; 
    }	
	
	if(pindexBest->nHeight > 13440)
    {
        nSubsidy = 0 * COIN; 
		return nSubsidy + nFees; 
    }
	
	if(pindexBest->nHeight > 16800)
    {
	...
	if(pindexBest->nHeight > 20640)
    {
        nSubsidy = 2 * COIN; 
		return nSubsidy + nFees; 
    }
	
	if(pindexBest->nHeight > 21120)
    {
        nSubsidy = 4 * COIN; 
		return nSubsidy + nFees; 
    }
	...
	if(pindexBest->nHeight > 22080)
    {
        nSubsidy = 6 * COIN; 
		return nSubsidy + nFees; 
    }
	if(pindexBest->nHeight > 22560)
    {
        nSubsidy = 7 * COIN; 
		return nSubsidy + nFees; 
    }	
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TheMermaidCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "02.05.2016 16:52";
        CTransaction txNew;
        txNew.nTime = 1462200731;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1462200731;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 1881524 : 1881524;
        ...
        assert(block.hashMerkleRoot == uint256("0xe202737f03dfde50b59733547e2884e606066b8571f6b5f62b46b416dd8318db"));
